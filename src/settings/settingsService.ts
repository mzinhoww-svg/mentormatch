...
const SELECT_ROW = `
  display_name, logo_url, primary_color, secondary_color, program_name, locale,
  status, allow_self_signup, default_mentor_capacity`;
    ...
    const res = await db.query<SettingsRow>(
      `INSERT INTO tenant_settings
         (tenant_id, display_name, logo_url, primary_color, secondary_color,
          program_name, locale, allow_self_signup, default_mentor_capacity)
       VALUES ($1, $2, $3, $4, $5, $6, COALESCE($7,'pt-BR'), COALESCE($8,true), COALESCE($9,3))
       ON CONFLICT (tenant_id) DO UPDATE SET
         ...
         program_name = COALESCE($6, tenant_settings.program_name),
         locale = COALESCE($7, tenant_settings.locale),
         allow_self_signup = COALESCE($8, tenant_settings.allow_self_signup),
         default_mentor_capacity = COALESCE($9, tenant_settings.default_mentor_capacity),
         updated_at = now()
